	...
	const categoryCheckboxHandler = (event) => {
		...
		isChecked
			...
			: dispatch({
					...
					payload: [
						...state.categoryFilters.filter((category) => category !== name)
					]
			  });
	};
	const genderCheckboxHandler = (event) => {
		...
		isChecked
			...
			: dispatch({
					...
					payload: [...state.genderFilters.filter((gender) => gender !== name)]
			  });
	};
				...
				<label htmlFor="price">
					...
					<input
						onChange={(event) => {
							...
							dispatch({ type: "setFilteredByPrice", payload: true });
						}}
						type="range"
						max={15000}
						min={500}
						...
						<li>
							<label>
								<input
									type="checkbox"
									value={gender}
									checked={state.genderFilters.includes(gender)}
									onClick={(event) => genderCheckboxHandler(event)}
								/>
								{gender}
							</label>
						</li>
							...
							<li>
								<label>
									<input
										...
										onChange={(event) => categoryCheckboxHandler(event)}
									/>
									{category.categoryName}
								</label>
							</li>
				...
				<ul>
					...
					{state.shoeSizes.map((size) => (
						<li>
							<label>
								<input
									checked={size.toString() === state.selectedSize}
									type="radio"
									name="size"
									value={size}
									onChange={(event) =>
